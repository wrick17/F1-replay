...
import { API_BASE_URL, DASHBOARD_PORT, SESSION_DELAY_MS, SUPPORTED_SESSION_TYPES, WORKER_BASE_URL } from "./config";
import { startDashboard } from "./dashboard";
import { initLogger } from "./logger";
import { buildMeetingIndex, fetchAllMeetings, findSessionByType, getEndedMeetings } from "./meetings";
...
const run = async () => {
  ...
  appendLog("Log file: ./warm-car-telemetry-cache.log");

  const meetings = getEndedMeetings(await fetchAllMeetings(openf1.fetchOpenF1), now);
  ...
  for (const year of years) {
    ...
    for (let index = 0; index < yearMeetings.length; index += 1) {
      ...
      for (const sessionType of SUPPORTED_SESSION_TYPES) {
        ...
        try {
          const cached = await worker.fetchCarTelemetryFromWorker(session.session_key);
          if (cached.status === "hit") {
            ...
            await warmSession(meeting, session, cached.uploadToken, {
              ...
              uploadCarTelemetryToWorker: worker.uploadCarTelemetryToWorker,
            });
            status.warmed += 1;
...
run().catch((error) => {
  const message = error instanceof Error ? error.message : String(error);
  console.error(`Car telemetry cache warmer failed: ${message}`);
  process.exitCode = 1;
});
