...
test("world uses race-local time and recorded weather at the replay cursor, including reverse seeks", () => {
  ...
  expect(at(start)).toMatchObject({ localHour: 15, rainfall: 0, windSpeed: 3, weatherKnown: true });
  expect(at(start + 60_000)).toMatchObject({ localHour: 15 + 1 / 60, rainfall: 1, windSpeed: 8, windDirection: 270 });
